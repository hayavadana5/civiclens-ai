...
const unwrap = <T>(d: any, key: string): T =>
  (d && typeof d === 'object' && !Array.isArray(d) && key in d ? d[key] : d) as T
...
export const getIssues = (): Promise<Issue[]> =>
  withFallback(async () => unwrap<Issue[]>((await http.get('/issues')).data, 'issues'), () => clone(db))

export const getIssue = (id: string): Promise<Issue> =>
  withFallback(async () => unwrap<Issue>((await http.get(`/issues/${id}`)).data, 'issue'), () => clone(find(id)))
...
export const getClusters = (): Promise<IssueCluster[]> =>
  withFallback(async () => unwrap<IssueCluster[]>((await http.get('/clusters')).data, 'clusters'), () => clone(groups))

export const getDashboardStats = (): Promise<DashboardStats> =>
  withFallback(async () => unwrap<DashboardStats>((await http.get('/dashboard/stats')).data, 'stats'), () => mockStats())
